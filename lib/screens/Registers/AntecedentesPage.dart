...
class _AntecedentesScreenState extends State<AntecedentesScreen> {
  ...
  final TextEditingController _tipoAntecedenteController =
  ...
  void initState() {
    ...
    _searchController.addListener(
        _filterAntecedentes); // Escucha los cambios en el campo de búsqueda
  }
  ...
  Future<void> _fetchAntecedentes() async {
    try {
      final fetchedAntecedentes =
          await AntecedentesServices().getAntecedentes();
      setState(() {
        antecedentes = fetchedAntecedentes;
        filteredAntecedentes =
            List.from(antecedentes); // Inicializamos la lista filtrada
        isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  void _filterAntecedentes() {
    setState(() {
      ...
      if (query.isEmpty) {
        filteredAntecedentes =
            List.from(antecedentes); // Si no hay búsqueda, mostramos todos
      } else {
        filteredAntecedentes = antecedentes.where((antecedente) {
          return antecedente['usuario']['nombre']
                  .toLowerCase()
                  .contains(query) ||
              antecedente['fecha_apertura'].toLowerCase().contains(query) ||
              antecedente['usuario']['id']
                  .toString()
                  .toLowerCase()
                  .contains(query);
                        ...
                        child: DataTable(
                          ...
                          rows: filteredAntecedentes.map((antecedente) {
                            return DataRow(
                              cells: <DataCell>[
                                DataCell(Text(
                                    antecedente['usuario']['id'].toString())),
                                DataCell(
                                    Text(antecedente['usuario']['nombre'])),
                                DataCell(Text(_formatDate(
                                    antecedente['fecha_apertura']))),
                                DataCell(
                                  Row(
                                    children: [
